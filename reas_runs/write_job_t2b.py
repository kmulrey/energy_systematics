...
proton_dir='/user/kmulrey/energy_systematics/energy_systematics/reas_runs/jobs_proton/'
iron_dir='/user/kmulrey/energy_systematics/energy_systematics/reas_runs/jobs_iron/'

base_dir='/user/kmulrey/energy_systematics/pipeline_reas/'
...
def write_file(event, azimuth, zenith, energy, seed, type):
    ...
    outfile.write('rm -rf /scratch/kmulrey/{0}/{1}/reas/$RUNNR/\n'.format(event,part_id))
    outfile.write('mkdir -p /scratch/kmulrey/{0}/{1}/reas/$RUNNR\n'.format(event,part_id))
    outfile.write('python /user/kmulrey/energy_systematics/energy_systematics/sibyll_runs/geninp.py --atmosphere --atmfile={6}/ATMOSPHERE_{4}.DAT -r $RUNNR -s {0} -u {1} -a {2} -z {3} -t {5} -d /scratch/kmulrey/{4}/{5}/reas/$RUNNR/ > /scratch/kmulrey/{4}/{5}/reas/$RUNNR/RUN$RUNNR.inp\n'.format(seed,energy,azimuth,zenith,event,part_id,run_dir))

    outfile.write('cp {2}/SIMgood.reas /scratch/kmulrey/{0}/{1}/reas/$RUNNR/SIM$RUNNR.reas\n'.format(event,part_id,run_dir))
    outfile.write('cp {2}/SIM{0}.list /scratch/kmulrey/{0}/{1}/reas/$RUNNR/SIM$RUNNR.list\n'.format(event,part_id,run_dir))
    outfile.write('cd /user/kmulrey/software/corsika-77100/run/\n')
    outfile.write('./corsika77100Linux_QGSII_urqmd_thin_coreas < //scratch/kmulrey/{0}/{1}/reas/$RUNNR/RUN$RUNNR.inp\n'.format(event,part_id))
    outfile.write('cd /scratch/kmulrey/{0}/{1}/reas/$RUNNR\n'.format(event,part_id))
