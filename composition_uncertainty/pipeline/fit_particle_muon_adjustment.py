...
def returnLORA(RUNNR,dir0,v,m):
    ...
    if os.stat(longfile).st_size>0:
        ...
        #hillas=0
        r_range=cors_r
        dens_range=cors_dens+cors_dens*(muon_dens/100)*m
...
for i in np.arange(len(proton_runnr)):
    ...
    part_int0,hillas0,d_ratio0=returnLORA(proton_runnr[i],proton_xmax_dir,2,0)
    part_int0,hillas0,d_ratio0_25=returnLORA(proton_runnr[i],proton_xmax_dir,2,0.25)
    part_int0,hillas0,d_ratio0_50=returnLORA(proton_runnr[i],proton_xmax_dir,2,0.5)

    if d_ratio0>0 and d_ratio0<10:
        interps_xmax_P.append(part_int0)
        hillas_xmax_P.append(hillas0)
        d_ratio_xmax_P.append(d_ratio0)
        d_ratio_xmax_P_25.append(d_ratio0_25)
        d_ratio_xmax_P_50.append(d_ratio0_50)

for i in np.arange(len(helium_runnr)):
    #for i in np.arange(len(helium_runnr)):
    part_int0,hillas0,d_ratio0=returnLORA(helium_runnr[i],helium_xmax_dir,2,0)
    part_int0,hillas0,d_ratio0_25=returnLORA(helium_runnr[i],helium_xmax_dir,2,0.25)
    part_int0,hillas0,d_ratio0_50=returnLORA(helium_runnr[i],helium_xmax_dir,2,0.5)

    if d_ratio0>0 and d_ratio0<10:
        interps_xmax_He.append(part_int0)
        hillas_xmax_He.append(hillas0)
        d_ratio_xmax_He.append(d_ratio0)
        d_ratio_xmax_He_25.append(d_ratio0_25)
        d_ratio_xmax_He_50.append(d_ratio0_50)

for i in np.arange(len(oxygen_runnr)):

    #for i in np.arange(len(oxygen_runnr)):
    part_int0,hillas0,d_ratio0=returnLORA(oxygen_runnr[i],oxygen_xmax_dir,2,0)
    part_int0,hillas0,d_ratio0_25=returnLORA(oxygen_runnr[i],oxygen_xmax_dir,2,.25)
    part_int0,hillas0,d_ratio0_50=returnLORA(oxygen_runnr[i],oxygen_xmax_dir,2,.5)

    if d_ratio0>0 and d_ratio0<10:
        interps_xmax_O.append(part_int0)
        hillas_xmax_O.append(hillas0)
        d_ratio_xmax_O.append(d_ratio0)
        d_ratio_xmax_O_25.append(d_ratio0_25)
        d_ratio_xmax_O_50.append(d_ratio0_50)

for i in np.arange(len(iron_runnr)):
    #for i in np.arange(len(iron_runnr)):
    part_int0,hillas0,d_ratio0=returnLORA(iron_runnr[i],iron_xmax_dir,2,0)
    part_int0,hillas0,d_ratio0_25=returnLORA(iron_runnr[i],iron_xmax_dir,2,.25)
    part_int0,hillas0,d_ratio0_50=returnLORA(iron_runnr[i],iron_xmax_dir,2,.5)

    if d_ratio0>0 and d_ratio0<10:
        interps_xmax_Fe.append(part_int0)
        hillas_xmax_Fe.append(hillas0)
        d_ratio_xmax_Fe.append(d_ratio0)
        d_ratio_xmax_Fe_25.append(d_ratio0_25)
        d_ratio_xmax_Fe_50.append(d_ratio0_50)
...
for i in np.arange(len(d_ratio_xmax_P)):
    outfile.write('{0} {1:.2f} {2:.2f}  {3:.2f}  {4:.2f}\n'.format(proton_id,hillas_xmax_P[i][2],d_ratio_xmax_P[i],d_ratio_xmax_P_25[i],d_ratio_xmax_P_50[i]))
for i in np.arange(len(d_ratio_xmax_He)):
    outfile.write('{0} {1:.2f} {2:.2f}  {3:.2f}  {4:.2f}\n'.format(helium_id,hillas_xmax_He[i][2],d_ratio_xmax_He[i],d_ratio_xmax_He_25[i],d_ratio_xmax_He_50[i]))
for i in np.arange(len(d_ratio_xmax_O)):
    outfile.write('{0} {1:.2f} {2:.2f}  {3:.2f}  {4:.2f}\n'.format(oxygen_id,hillas_xmax_O[i][2],d_ratio_xmax_O[i],d_ratio_xmax_O_25[i],d_ratio_xmax_O_50[i]))
for i in np.arange(len(d_ratio_xmax_Fe)):
    outfile.write('{0} {1:.2f} {2:.2f}  {3:.2f}  {4:.2f}\n'.format(iron_id,hillas_xmax_Fe[i][2],d_ratio_xmax_Fe[i],d_ratio_xmax_Fe_25[i],d_ratio_xmax_Fe_50[i]))
