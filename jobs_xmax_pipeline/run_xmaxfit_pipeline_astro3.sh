...
rm /vol/astro3/lofar/sim/pipeline/log/failed.txt

# Run cr_physics pipeline
/usr/bin/python -u $PYCRTOOLS/extras/get_event_ids.py --host coma00.science.ru.nl --user crdb --password crdb --dbname crdb --simulation-status=LORASIM_DONE > $HOME/cr_xmaxfit_lorasim_done_astro3.txt
...
sleep 5
JOBS=$(sed -n '$=' $HOME/cr_xmaxfit_lorasim_done_astro3.txt)

echo "processing $JOBS astro7 events (LORASIM_DONE)"

/usr/local/slurm/bin/sbatch -p normal --array 1-${JOBS}%${MAX_SIMULTANEOUS_JOBS} cr_xmaxfit_astro3.sh
